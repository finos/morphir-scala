...
import Extractors.ImplicitArgs
...
/**
 * Printer for fully elaborated representation of the source code. Futher customized based on SourceCode in the Dotty
 * repo. In many situations if symbol.owner cannot be found the compiler will fail with a NoDenotation.owner error. This
 * typically happens when there is some kind of upstream problem which is the cause of the real issue e.g. a foo.bar
 * call where the `bar` doesn't actually exist. That means that the NoDenotation.owner error is just noise. Therefore
 * the show___ methods now return a try that catches a custom NoDenotationException which callers of this object can
 * deal with.
 */
object SourceCode {
  ...
  private class SourceCodePrinter[Q <: Quotes & Singleton](
    ...
    def printTree(tree: Tree)(using elideThis: Option[Symbol] = None): this.type = tree match {
      ...
      case cdef @ ClassDef(name, DefDef(_, paramss, _, _), parents, self, stats) =>
        ...
        if (name == "package$") {
          ...
          this += highlightKeyword("object ") += highlightTypeDef(name.stripSuffix("$"))
            ...
            d.symbol.flags.is(Flags.Synthetic) &&
              (d match {
                case DefDef("apply" | "unapply" | "writeReplace", _, _, _)
                    if d.symbol.ownerSafe.flags.is(Flags.Module) =>
                  true
      ...
      case TypeApply(fn, args) =>
        ...
        val noTermParams =
          (for {
            ...
            firstParamList <- fn.symbol.paramSymss.find(params =>
              params.headOption.exists(p => p.isValDef && !p.flags.is(Flags.Implicit))
            )
    ...
    private def printTargsDefs(targs: List[(TypeDef, TypeDef)], isDef: Boolean = true)(using
    ...
    private def printTargDef(arg: (TypeDef, TypeDef), isMember: Boolean = false, isDef: Boolean = true)(using
        elideThis: Option[Symbol]
    ): this.type = {
     ...
     *
     * @param elideThis
     *   Shoud printing elide `C.this` for the given class `C`? None means no eliding.
     *
     * Self type annotation and types in parent list should elide current class prefix `C.this` to avoid type checking
     * errors.
     */
      ...
      case Inferred() =>
        ...
        def printTypeAndAnnots(tpe: TypeRepr): this.type = tpe match {
          case AnnotatedType(tp, annot) =>
            ...
            printAnnotation(annot)
          case tpe: TypeRef
              if tpe.typeSymbol == Symbol.requiredClass("scala.runtime.Null$") || tpe.typeSymbol == Symbol
    ...
    private def printMethodicTypeParams(paramNames: List[String], params: List[TypeRepr])(using
        elideThis: Option[Symbol]
    ): Unit = {
    ...
    private object Types {
      ...
      object Sequence {
        def unapply(tpe: TypeRepr): Option[TypeRepr] = tpe match {
          case AppliedType(seq, (tp: TypeRepr) :: Nil)
              if seq.typeSymbol == Symbol.requiredClass("scala.collection.Seq") || seq.typeSymbol == Symbol
                .requiredClass("scala.collection.immutable.Seq") =>
            Some(tp)
          case _ => None
        }
      }
      ...
      object Repeated {
        def unapply(tpe: TypeRepr): Option[TypeRepr] = tpe match {
          case AppliedType(rep, (tp: TypeRepr) :: Nil) if rep.typeSymbol == Symbol.requiredClass("scala.<repeated>") =>
            Some(tp)
          case _ => None
        }
      }
