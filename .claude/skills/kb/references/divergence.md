...
`kb check` finds mechanical inconsistency — broken links, missing fields, index drift, stale pins. It cannot find the
kind of divergence that actually degrades a knowledge base: two concepts that assert incompatible things, or a
concept that no longer matches the source it claims to summarize.
...
```bash
.claude/skills/kb/kb check --verbose --json
```
...
```bash
.claude/skills/kb/kb show --path /naming.md --bundle morphir-ir-v3 --json | jq -r '.sources[]'
```

```bash
git -C .refs/finos/morphir diff <pinned-sha>..HEAD -- docs/spec/ir/morphir-ir-specification.md
```

An empty diff means the pin is merely old, not wrong — re-pin and move on. A non-empty diff needs reading against the
concept's claims.
...
```bash
.claude/skills/kb/kb search --query "format version"
```
...
```bash
.claude/skills/kb/kb search --type "Specification Section" --json | jq -r '.results[] | "\(.bundle)\(.path)  \(.description)"'
```
...
```bash
.claude/skills/kb/kb search --query divergence --body
```
...
**Say which source is authoritative.** Often the rule already exists — for the Morphir bundles, `finos/morphir`
`docs/` is authoritative for the specification and `finos/morphir-elm` for how v3 actually behaves. The group's
