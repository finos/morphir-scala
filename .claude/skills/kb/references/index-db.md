...
`kb index` builds a SQLite database over the knowledge base so that full-text search, link-graph traversal and
frontmatter faceting are single queries rather than a re-parse of every file.
...
```bash
.claude/skills/kb/kb query --sql "SELECT from_bundle, from_path, line FROM v_backlink JOIN doc d ON d.id = v_backlink.doc_id WHERE d.bundle_path = '/naming.md'"
```

**Concepts nothing links to:**

```bash
.claude/skills/kb/kb query --sql "SELECT bundle, bundle_path, title FROM v_orphan"
```

**Everything drawn from one upstream repository, newest pin first:**

```bash
.claude/skills/kb/kb query --sql "SELECT DISTINCT s.commit_sha, count(*) AS docs FROM source s WHERE s.repo = 'morphir-elm' GROUP BY s.commit_sha"
```

**Tag co-occurrence — which tags travel together:**

```bash
.claude/skills/kb/kb query --sql "SELECT a.tag, b.tag, count(*) AS n FROM tag a JOIN tag b ON a.doc_id = b.doc_id AND a.tag < b.tag GROUP BY 1,2 HAVING n > 3 ORDER BY n DESC LIMIT 15"
```

**Draft concepts past their review date:**

```bash
.claude/skills/kb/kb query --sql "SELECT bundle, bundle_path, stale_after FROM v_concept WHERE status = 'draft' AND stale_after < date('now') ORDER BY stale_after"
```

**Where a term appears, by document, with the heading it sits under** — combine FTS with `heading`:

```bash
.claude/skills/kb/kb query --sql "SELECT d.bundle_path, h.text FROM doc_fts f JOIN doc d ON d.id = f.rowid JOIN heading h ON h.doc_id = d.id WHERE doc_fts MATCH 'incompleteness' AND h.level = 2"
```

**Cross-bundle link traffic** — which bundles reference which:

```bash
.claude/skills/kb/kb query --sql "SELECT sb.label AS from_bundle, tb.label AS to_bundle, count(*) AS n FROM link l JOIN doc s ON s.id = l.doc_id JOIN bundle sb ON sb.id = s.bundle_id JOIN doc t ON t.id = l.target_doc_id JOIN bundle tb ON tb.id = t.bundle_id GROUP BY 1,2"
```

## Relationship to `kb check`
