...
object SquireDoctor:
  ...
  def run(
      ...
      platform: SquireEnv.Platform = SquireEnv.LivePlatform
  ): DoctorReport < (Async & Sync & Abort[SquireError]) =
        ...
        platform.probeDaemon(port) match
          ...
          case SquireEnv.DaemonProbe.Sandbox(detail) =>
            Finding(
              "mill_daemon",
              "SANDBOX",
              s"JVM socket is blocked on mill daemon port $port: $detail; use ./mill --no-server",
              true
            )
          case SquireEnv.DaemonProbe.Refused(detail) =>
            Finding(
              "mill_daemon",
              "REFUSED",
              s"mill daemon port $port is not accepting connections: $detail; plain ./mill will restart it",
              false
            )
  ...
  private def setupFinding(root: Path): Finding =
    ...
    if Files.exists(setup.toJava) && Files.readString(setup.toJava).contains("bun install --ignore-scripts") &&
      (!Files.exists(packageJson.toJava) || !rootMorphirElmDependency(Files.readString(packageJson.toJava)))
    then
      ...
      Finding(
        "setup",
        "MISSING",
        "setup must use bun install --ignore-scripts and leave Morphir Elm provisioning to Mill",
        true
      )
  ...
  private def mainClassFinding(root: Path): Finding =
    val packageYaml = root / "morphir" / "langkit" / "elm" / "compiler" / "mep" / "package.mill.yaml"
    if Files.exists(packageYaml.toJava) &&
      """(?m)^\s*mainClass:\s*morphir\.langkit\.elm\.compiler\.mep\.Main\s*$""".r
        .findFirstIn(Files.readString(packageYaml.toJava)).nonEmpty
    then
      Finding(
        "main_class_task",
        "OK",
        "Elm MEP mainClass is configured in morphir/langkit/elm/compiler/mep/package.mill.yaml",
        false
      )
    else
      Finding(
        "main_class_task",
        "NOT_FOUND",
        "Elm MEP mainClass is missing from morphir/langkit/elm/compiler/mep/package.mill.yaml",
        true
      )
  ...
  private def millMorphirFinding(root: Path): Finding =
    ...
    val wiring = List(
      ...
      (pluginRoot / "integration" / "test" / "src" / "org" / "finos" / "morphir" / "mill" /
        "PublishedPluginIntegrationTests.scala") -> List("COURSIER_REPOSITORIES", "millExecutable"),
      (pluginRoot / "integration" / "resources" / "published-consumer" / "build.mill") ->
        List("MORPHIR_PUBLISHED_TEST_REPOSITORIES")
    )
    ...
    if missing.nonEmpty then
      Finding(
        "mill_morphir",
        "MISSING",
        s"Mill Morphir plugin modules are missing: ${missing.mkString(", ")}; verify ./mill resolve 'mill-plugins.morphir.__'",
        true
      )
      ...
      Finding(
        "mill_morphir",
        "MISSING",
        "task-local plugin repository resolution is not wired; verify ./mill mill-plugins.morphir.integration.test",
        true
      )
    else
      Finding("mill_morphir", "OK", "Mill Morphir plugin modules and task-local repository wiring are present", false)
      ...
      inspection.corrupt match
        case Some(entry) =>
          Finding(
            "acquisition_cache",
            "CORRUPT",
            s"corrupt acquisition cache entry: $entry; reacquire online with MORPHIR_NODE_OFFLINE=false",
            true
          )
        case None if inspection.bounded.nonEmpty =>
          Finding(
            "acquisition_cache",
            "NOTICE",
            s"acquisition cache diagnostic was bounded: ${inspection.bounded.mkString("; ")}",
            false
          )
  ...
  private def absoluteEnvironmentPath(platform: SquireEnv.Platform, name: String): Option[Path] =
    platform.environment.get(
      name
    ).filter(_.nonEmpty).flatMap(value => Try(Path(value)).toOption).filter(_.toJava.isAbsolute)
  ...
  private def metabuildFinding(root: Path): Finding =
    val sourceCandidates =
      List(root / "build.mill", root / ".mill-version").filter(path => Files.isRegularFile(path.toJava)) ++
    ...
    if compiled.isEmpty then Finding("metabuild", "NOTICE", "Mill metabuild has no compiled output yet", false)
      ...
      Finding(
        "metabuild",
        "STALE",
        "Mill metabuild compilation is stale; verify ./mill resolve 'mill-plugins.morphir.__'",
        true
      )
