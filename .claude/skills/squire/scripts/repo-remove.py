...
By default this removes the on-disk entry (.refs/<name>) in addition to the
manifest entry. Pass --keep-files to only remove from the manifest.

For worktrees the git worktree is pruned from the source repo. For symlinks
the symlink is unlinked. For clones the directory is deleted.
"""
...
def remove_worktree(entry):
    ...
    dest = (REFS_DIR / entry["name"]).resolve()
    if source.exists():
