...
- `REFUSED` or `NO_DAEMON` → daemon not running; plain `./mill` will start one, or use `./mill --no-server`.
...
- the Elm MEP YAML-owned `mainClass` setting
...
### 2. Elm MEP main class missing

The process extension declares its entry point in
`morphir/langkit/elm/compiler/mep/package.mill.yaml`:

```yaml
mainClass: morphir.langkit.elm.compiler.mep.Main
```

Restore that declaration if the doctor reports `main_class_task: NOT_FOUND`.
The root `morphir/package.mill.yaml` contains libraries only; the general Scala CLI has retired.
