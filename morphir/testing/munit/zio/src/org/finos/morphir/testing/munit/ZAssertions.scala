...
trait ZAssertions {
  ...
  /**
   * Asserts that `ZIO[R, E, String]` has no difference with expected string. Pretty prints diff unlike just
   * `assertEqualsZ`.
   *
   * {{{
  ...
  private def runIntercept[E <: Throwable](
    ...
    exit match {
      ...
      case Exit.Failure(cause) =>
        ...
        e match {
          case Some(error: FailExceptionLike[?]) if !E.runtimeClass.isAssignableFrom(error.getClass()) =>
            ZIO.fail(error)
