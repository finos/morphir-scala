...
transport-blind service contract they consume. It is the shared UI library behind the local web
host started by `morphir server` (see [`morphir/web/renderer`](../web/renderer) and
[`morphir/web/server`](../web/server)). The Morphir desktop application, which previously mounted
the same UI value in an Electron renderer, has moved to
...
Settings includes a GitHub.com connection panel used by the browser host. Paste a personal
