 ...
 * https://github.com/zio/zio-prelude/blob/754ced18f21573d1bc128715ceb24899d1e67353/macros/shared/src/main/scala/zio/prelude/Assertion.scala
 */
...
/**
 * An `Assertion[A]` is essentially a composable predicate from `A => Boolean`. They can be composed with standard
 * Boolean operators of `&&`, `||` and `!`. This is primarily intended to be used with `Newtype` and `Subtype`,
 * enhancing them with compile-time time validation.
 *
 * For example, if you'd like to validate that a particular Int is precisely 4 digits long, you can create the following
