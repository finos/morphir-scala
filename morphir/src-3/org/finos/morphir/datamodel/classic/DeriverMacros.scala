...
import org.finos.morphir.datamodel.classic.UnionType
    ...
    def failNotProductOrSum() =
      report.errorAndAbort(
        s"Cannot summon generic Deriver for the type (was not a Product or Sum): ${TypeRepr.of[T].widen
            .show} from `summonDeriver` (flags: ${TypeRepr.of[T].typeSymbol.flags.show}). Have you imported org.finos.morphir.datamodel.classic.{given, _}"
      )
      ...
      case None        =>
        ...
        if (Expr.summon[Mirror.ProductOf[T]].nonEmpty) {
          ...
          genericDeriver match {
            ...
            case _           =>
              report.errorAndAbort(
                s"Cannot summon specific or generic Sum Deriver for the sum type: ${tpe.widen.show} from `summonDeriver` (flags: ${flags.show}). Have you imported org.finos.morphir.datamodel.classic.{given, _}"
              )
          }
        } else {
          failNotProductOrSum()
    ...
    def failNotProduct() =
      report.errorAndAbort(
        s"Cannot summon generic Deriver for the type (was not a Product): ${TypeRepr.of[T].widen
            .show} from `summonProductDeriver` (flags: ${TypeRepr.of[T].typeSymbol.flags.show}). Have you imported org.finos.morphir.datamodel.classic.{given, _}"
      )
    val tpe   = TypeRepr.of[T]
    val flags = tpe.typeSymbol.flags
