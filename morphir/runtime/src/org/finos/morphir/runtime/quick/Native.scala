...
object DictSDK {
  ...
  val diff: SDKValue = SDKValue.SDKNativeInnerFunction {
    NativeFunctionSignatureAdv.Fun2 {
      // === ELM Function ===
      // diff : Dict comparable a -> Dict comparable b -> Dict comparable a
      // Keep a key-value pair when its key does not appear in the second dictionary.
      (evaluator: InvokeableEvaluator) => (dict1Raw: RTValue, dict2Raw: RTValue) =>
        {
          val dict1 = dict1Raw.coerceMap.value
          val dict2 = dict2Raw.coerceMap.value
  ...
  val intersect: SDKValue = SDKValue.SDKNativeInnerFunction {
    NativeFunctionSignatureAdv.Fun2 {
      // === ELM Function ===
  ...
  val union: SDKValue = SDKValue.SDKNativeInnerFunction {
    NativeFunctionSignatureAdv.Fun2 {
      // === ELM Function ===
  ...
  val foldl: SDKValue = SDKValue.SDKNativeInnerFunction {
    NativeFunctionSignatureAdv.Fun3 {
      ...
      (evaluator: InvokeableEvaluator) => (f: RTValue, b: RTValue, dictRaw: RTValue) =>
        {
          val dict = dictRaw.coerceMap.value
          val result = dict.foldLeft(b) {
            case (acc, (k, v)) =>
              evaluator.handleApplyResult(Type.UType.Unit(()), f, k, v, acc)
          }
          result
        }
    }
  ...
  val foldr: SDKValue = SDKValue.SDKNativeInnerFunction {
    NativeFunctionSignatureAdv.Fun3 {
      // === ELM Function ===
