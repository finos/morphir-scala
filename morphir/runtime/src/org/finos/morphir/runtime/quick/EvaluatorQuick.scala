...
object EvaluatorQuick {
  ...
  def typeToConcept(tpe : Type.Type[Unit], dist : Library) : Concept = {
    tpe match
      case TT.ExtensibleRecord(attributes, name, fields) => throw new UnsupportedType("Extensible records not supported for DDL")
      case TT.Function(attributes, argumentType, returnType) => throw new UnsupportedType("Functiom types not supported for DDL")
      case TT.Record(attributes, fields) => Concept.Record(fields.map(field => (Label(field.name.toCamelCase), typeToConcept(field.data, dist))).toList)
      ...
      case TT.Reference(attributes, FQString("Morphir.SDK:List:list"), Chunk(elementType)) => Concept.List(typeToConcept(elementType, dist))
      case TT.Reference(attributes, FQString("Morphir.SDK:Maybe:maybe"), Chunk(elementType)) => Concept.Optional(typeToConcept(elementType, dist))
      ...
      case TT.Reference(attributes, typeName, typeParams) => {
        ...
        lookedUp.getOrElse(throw new Exception (s"Could not find spec for $typeName")) match {
          case Type.Specification.TypeAliasSpecification(params, expr) => Concept.Alias(typeName.toString, typeToConcept(expr, dist))
          case Type.Specification.CustomTypeSpecification(params, ctors) => {
            val cases = ctors.toMap.toList.map{case (caseName, args) => {
              val argTuples = args.map{case (argName : Name, argType : Type.UType) => (EnumLabel.Named(argName.toCamelCase), typeToConcept(argType, dist))}
  ...
  def resultAndConceptToData(result : Result[Unit, Type.UType], concept : Concept) : Data = {
    (concept, result) match
      case (Concept.Record(fields), Result.Record(elements)) => {
        if (fields.length != elements.size) {
          ...
          val tuples = fields.map { case (Label(name), innerConcept) => {
            val value = elements.getOrElse(Name(name), throw new MissingField(s"Type expected $name but not found in $elements"))
      ...
      case (Concept.Optional(elementShape), Result.ConstructorResult(
      ...
      case (mapConcept @ Concept.Map(keyConcept, valConcept), Result.MapResult(elements)) => {
        val inners = elements.toList.map{case (key, value) => (resultAndConceptToData(key, keyConcept), resultAndConceptToData(value, valConcept))}
      ...
      case (enumConcept @ Concept.Enum(name, cases), Result.ConstructorResult(fqName, args)) => {
        ...
        val fields = fieldMap.getOrElse(fqName.localName.toCamelCase, throw new ResultDoesNotMatchType(s"Failed to find constructor ${fqName.localName} among ${fieldMap.keys}"))
        if (args.length != fields.length) throw new ResultDoesNotMatchType(s"Parameter lengths differ between $args and $fields")
        else {
          val zipped = args.zip(fields)
          val argData = zipped.map{ case (innerResult, (argName, argConcept)) => (argName, resultAndConceptToData(innerResult, argConcept))}
      ...
      case (Concept.Tuple(conceptElements), Result.Tuple(resultElements)) => {
        ...
        if (conceptElements.length != listed.length) {
          throw new ResultDoesNotMatchType(s"Tuple type elements $conceptElements of different length than result $resultElements")
        } else {
          val inners = conceptElements.zip(listed).map { case (innerConcept, innerResult) => resultAndConceptToData(innerResult, innerConcept) }
          Data.Tuple(inners)
        }
      }
      case (Concept.Unit, Result.Unit()) => Data.Unit
      case (badType, badResult) => throw new ResultDoesNotMatchType(s"Could not match type $badType with result $badResult")
