...
object EvaluatorQuick {
  ...
  def typeToConcept(tpe: Type.Type[Unit], dists: Distributions, boundTypes: Map[Name, Concept]): Concept =
    tpe.concept(dists, boundTypes) match {
      // TODO: This is a stand-in to make sure tests work; will properly merge errors before merging
      case Left(Unsupported(msg)) => throw new UnsupportedType(tpe, msg)
