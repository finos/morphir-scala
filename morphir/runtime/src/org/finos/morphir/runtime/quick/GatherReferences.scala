...
object GatherReferences {
  ...
  def fromStore(store : Store[Unit, UType]) : ReferenceSet = {
    store.definitions.map{case (name, value) => value match {
  ...
  def fromDistributions(dists : Distribution*): ReferenceSet = {
    dists.foldLeft(ReferenceSet.empty)((acc : ReferenceSet, dist : Distribution) => acc ++ (dist match{
      case l : Library => fromLibrary(l)
    }))
  }
  def fromLibrary(lib : Library) : ReferenceSet = {
    val packageName = lib.packageName
    val valueReferences : List[ReferenceSet] = lib.packageDef.modules.toList.flatMap { case (moduleName, accessControlledModule) =>
  ...
  def loop(ir: TypedValue): ReferenceSet = {
    ...
    def fold(stuff: Iterable[TypedValue]): ReferenceSet = stuff.foldLeft(empty) { case (acc, next) => acc ++ loop(next) }
    ...
    ir match {
      ...
      case Value.PatternMatch(_, value, cases) => loop(value) ++ cases.foldLeft(empty) { case (acc, (pattern, value)) => acc ++ loop(value) ++ patternLoop(pattern) }
    ...
    def patternLoop(pattern : Pattern[UType]) : ReferenceSet = {
      val empty = ReferenceSet.empty;
      def fold(stuff: Chunk[Pattern[UType]]): ReferenceSet = stuff.foldLeft(empty) { case (acc, next) => acc ++ patternLoop(next) }
      pattern match {
