...
object EvaluatorQuickSpec extends MorphirBaseSpec {
  ...
  private def unwrap(value: Data): Any = value match {
    case Data.Boolean(value) => value
    case Data.Int16(value)   => value
    case Data.Int32(value)   => value
    case Data.Int64(value)   => value
    case Data.Float(value)   => value
    case Data.String(value)  => value
    case Data.Tuple(values)  =>
      values.map(unwrap) match {
        case first :: second :: Nil          => (first, second)
        case first :: second :: third :: Nil => (first, second, third)
        case other                           => other
      }
    case value: Data.List => value.values.map(unwrap)
    case other            => other
  }
  ...
  def spec = suite("Basic Evaluation")(
    ...
    suite("Destructure")(
      test("Should work with tuples") {
        ...
        val value: TypedValue = V.destructure(
          attributes = ir.sdk.String.stringType,
          pattern = pat,
          valueToDestruct = c,
          inValue = in
        )
      ...
      test("Should be able to create a tuple from the first 2 elements in a list in reverse order") {
        ...
        val value: TypedValue =
          V.destructure(pat, c, in) :> T.tupleVar(ir.sdk.Basics.floatType, ir.sdk.Basics.intType)
        val result = eval(value)
