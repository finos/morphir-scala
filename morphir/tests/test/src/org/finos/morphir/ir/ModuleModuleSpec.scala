...
//        assertTrue(moduleDef.toSpecification == expected)
      ...
      test("Can look up values") {
        assertTrue(
          moduleDef.lookupValueDefinition(Name("val")) == Some(
            Value.Definition(outputType = sdk.String.stringType, body = Value.string(sdk.String.stringType, "string"))
          )
        )
      },
      test("Can be erased") {
    ...
    suite("Module Specification")(
      test("Can look up values") {
        ...
        assertTrue(
          result.isDefined && result.get.inputs.size == 2 && result.get.output == Type.variable("WholeNumbers")
        )
      },
