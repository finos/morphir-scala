...
commonmark-java. Frontmatter uses Kyo's `kyo-schema` and `kyo-schema-yaml`; private wire types use `Option` because
the schema codec does not derive `Maybe`, then convert at the public boundary to `Maybe`.
