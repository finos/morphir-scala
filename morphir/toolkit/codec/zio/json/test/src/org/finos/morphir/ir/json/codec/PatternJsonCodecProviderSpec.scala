...
object PatternJsonCodecProviderSpec extends MorphirJsonBaseSpec with MorphirIRDeriveGen with MorphirJsonSupport {
  ...
  def spec = suite("PatternJsonCodecProviderSpec")(
    // goldenTest(PatternGen.pattern(LiteralGen.literal))
    goldenTest(DeriveGen[Pattern[Literal]])
  )
}
