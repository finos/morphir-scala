...
trait MorphirJsonEncodingSupport {
  ...
  implicit def literalDecimalEncoder: JsonEncoder[DecimalLiteral] = Json.encoder.contramap[DecimalLiteral] { literal =>
    Json.Arr(Json.Str("DecimalLiteral"), Json.Str(literal.value.toString))
  }
  ...
  implicit def literalWholeNumberEncoder: JsonEncoder[WholeNumberLiteral] =
    Json.encoder.contramap[WholeNumberLiteral] { literal =>
      Json.Arr(Json.Str("WholeNumberLiteral"), Json.Num(literal.value))
    }
  ...
  implicit val morphirIRVersionEncoder: JsonEncoder[MorphirIRVersion] = JsonEncoder.int.contramap(_.versionNumber.toInt)
