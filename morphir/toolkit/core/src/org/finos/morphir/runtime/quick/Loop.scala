...
object Loop {
    ...
    functionValue match {
      ...
      case Result.DefinitionFunction(body, arguments, curried, closingContext) =>
        arguments match {
          ...
          case Nil =>
            throw FunctionWithoutParameters("Tried to apply definition function with no un-applied arguments (should not exist)")
      ...
      case Result.ConstructorFunction(name, arguments, curried) =>
        arguments match {
          ...
          case Nil =>
            throw FunctionWithoutParameters(s"Tried to apply to constructor function with no arguments (should not exist)")
  ...
  def handleConstructor[TA, VA](va: VA, name: FQName, store: Store[TA, VA]): Result[TA, VA] =
    store.getCtor(name) match {
      ...
      case None => throw ConstructorNotFound(s"$name not found in constructor store. Store contents: ${store.ctors.keys}")
    }
