...
object EvaluatorQuickSpec extends MorphirBaseSpec {
  ...
  def spec = suite("Basic Evaluation")(
    ...
    suite("NativeFunction Spec")(
      test("Addition") {
        val reference = V.reference(FQName.fromString("SDK.Basics.Int.Plus"))//:> T.function(ir.sdk.Basics.intType, T.function(ir.sdk.Basics.intType, ir.sdk.Basics.intType))
        val curried = V.apply(
          reference,
...
suite("Destructure")(
    test("Should work with tuples") {
      ...
      val pat = V.tuplePattern(attributes, asAlias(ir.sdk.Basics.intType, "a"), asAlias(ir.sdk.String.stringType, "b"))
