...
/**
 * Ported from ZIO:
 * https://github.com/zio/zio/blob/series/2.x/core/shared/src/main/scala/zio/Chunk.scala
 */
...
sealed abstract class Chunk[+A] extends ChunkLike[A] with Serializable { self =>
  ...
  protected[prelude] def toArray[A1 >: A](n: Int, dest: Array[A1]): Unit =
...
object Chunk extends ChunkFactory with ChunkPlatformSpecific {
   ...
   */
  private[prelude] def classTagOf[A](chunk: Chunk[A]): ClassTag[A] =
    chunk match {
  ...
  private final case class AppendN[A](start: Chunk[A], buffer: Array[AnyRef], bufferUsed: Int, chain: AtomicInteger)
    ...
    override protected[prelude] def toArray[A1 >: A](n: Int, dest: Array[A1]): Unit = {
  ...
  private[prelude] sealed abstract class Arr[A] extends Chunk[A] with Serializable { self =>
  ...
  private[prelude] trait BitOps[T] {
    def zero: T
    def one: T
    def reverse(a: T): T
    def <<(x: T, n: Int): T
  ...
  private[prelude] sealed abstract class BitChunk[T](
      chunk: Chunk[T],
      val bits: Int
  ...
  private[prelude] final case class BitChunkByte(bytes: Chunk[Byte], minBitIndex: Int, maxBitIndex: Int)
  ...
  private[prelude] final case class BitChunkInt(
      ints: Chunk[Int],
  ...
  private[prelude] final case class BitChunkLong(
      longs: Chunk[Long],
      endianness: BitChunk.Endianness,
      minBitIndex: Int,
      maxBitIndex: Int
  ...
  private[prelude] final case class ChunkPackedBoolean[T](
    ...
    override protected[prelude] def toArray[T1 >: T](n: Int, dest: Array[T1]): Unit = {
      var i = n
      while (i < length) {
        dest(i + n) = self.apply(i)
        i += 1
  ...
  /**
   * A `ChunkIterator` is a specialized iterator that supports efficient iteration over chunks. Unlike a normal
   * iterator, the caller is responsible for providing an `index` with each call to `hasNextAt` and `nextAt`. By
   * contract this should be `0` initially and incremented by `1` each time `nextAt` is called. This allows the caller
   * to maintain the current index in local memory rather than the iterator having to do it on the heap for array backed
   * chunks.
   */
