...
object TextRun:
  trait Renderer:
    def apply(run:TextRun):String

  object Renderer:
    given Renderer = run => run.rawValue
  inline def apply(text:String):TextRun = raw(text)
  def apply(text:String)(findMarkers: String => Seq[Int]):TextRun =
    val len = text.length
    val markers = findMarkers(text).filter(n => n >= 0 && n < len)
    TextRun.Marked(text, markers.toArray)

  def raw(text:String):TextRun = TextRun.Raw(text)
end TextRun
