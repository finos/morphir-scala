...
import scala.quoted.*
import morphir.prelude.ConsoleUtils.*
...
abstract class NewtypeCustom[A] {
  ...
  /**
   * Function that will used to check runtime values before lifting them into the newtype. Should be consistent with
   * `validateInline`.
   */
  protected def validate(value: A): Either[AssertionError, Unit]

  /**
   * Function that will be used to check compile-time values before lifting them into the newtype. Should be consistent
   * with `validate`.
   */
