...
//   def serviceWith[R]: ServiceWithPartiallyApplied[R] = new ServiceWithPartiallyApplied[R]
//   def serviceWithAction[R]                           = ???
...
//     def apply[A](f: R => A)(implicit tag: Tag[R]): RTAction[R, Nothing, A] =
//       RTAction(ZPure.serviceWithPure(r => ZPure.succeed[RTExecutionContext, A](f(r))))
//   }

//   final class ServiceWithActionPartiallyApplied[R](private val dummy: Boolean = true) extends AnyVal {
//     def apply[E, A](
//         f: R => RTAction[R, E, A]
//     )(implicit tag: Tag[R]): RTAction[R, E, A] =
//       RTAction(ZPure.environmentWithPure(env => f(env.get).computation))
//   }
// }
