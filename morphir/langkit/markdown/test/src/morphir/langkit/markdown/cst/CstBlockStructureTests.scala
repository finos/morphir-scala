...
class CstBlockStructureTests extends Test[Any]:
  ...
  private def blocks(source: String): Chunk[MdCstNode] = CstParser.parse(source).children

  private def tokenTexts(node: MdCstNode): Chunk[String] =
    node.childNodes.collect { case MdCstNode.Token(text, _) => text }

  "graduated blocks" - {

    "a thematic break is a typed node holding one token" in {
      blocks("***\n") match
        case Chunk(MdCstNode.ThematicBreak(_, _), MdCstNode.Verbatim("\n", _)) => assert(true)
    ...
    "an ATX heading keeps its level and its marker token" in {
      blocks("## Hi\n").head match
        case MdCstNode.AtxHeading(level, _, _) =>
    ...
    "an ATX heading keeps its closing sequence as a token" in {
      blocks("# Hi #\n").head match
        case MdCstNode.AtxHeading(_, _, _) =>
          assert(tokenTexts(blocks("# Hi #\n").head).size == 2)
        case other => assert(false, s"expected AtxHeading, got $other")
    }
    ...
    "a setext heading keeps its underline as a token" in {
      blocks("Hi\n===\n").head match
        case MdCstNode.SetextHeading(level, _, _) =>
    ...
    "a closed fence is token, text, token" in {
      blocks("```scala\ncode\n```\n").head match
        case MdCstNode.FencedCode(children, _) =>
          children.toSeq match
            case Seq(MdCstNode.Token(open, _), MdCstNode.Text(_, _), MdCstNode.Token(close, _)) =>
    ...
    "an unterminated fence has no closing token" in {
      blocks("```\ncode\n").head match
        case MdCstNode.FencedCode(children, _) =>
          assert(children.collect { case t: MdCstNode.Token => t }.size == 1)
        case other => assert(false, s"expected FencedCode, got $other")
    }

    "indented code is typed, interior still verbatim" in {
      blocks("    code\n").head match
        case MdCstNode.IndentedCode(_, _) => assert(true)
    ...
    "a paragraph is typed, interior still verbatim until inlines graduate" in {
      blocks("plain prose\n").head match
        case MdCstNode.Paragraph(_, _) => assert(true)
  ...
  "graduated containers" - {
    ...
    "a two-line quote's interior marker is a token inside the paragraph" in {
      blocks("> a\n> b\n").head match
        case MdCstNode.BlockQuote(children, _) =>
          val para = children.collect { case p: MdCstNode.Paragraph => p }
          assert(para.size == 1)
    ...
    "a heading inside a quote is typed inside the quote" in {
      blocks("> # H\n").head match
        case MdCstNode.BlockQuote(children, _) =>
          assert(children.exists {
            case MdCstNode.AtxHeading(HeadingLevel.One, _, _) => true
            case _                                            => false
          })
        case other => assert(false, s"expected BlockQuote, got $other")
    }
    ...
    "a nested quote is a quote inside a quote" in {
      blocks("> > deep\n").head match
        case MdCstNode.BlockQuote(children, _) =>
          assert(children.exists {
            case _: MdCstNode.BlockQuote => true
            case _                       => false
          })
        case other => assert(false, s"expected BlockQuote, got $other")
    }
    ...
    "a blank line between items makes the list loose" in {
      blocks("- a\n\n- b\n").head match
        case MdCstNode.BulletList(_, tight, _, _) => assert(!tight)
        case other                                => assert(false, s"expected BulletList, got $other")
    }
    ...
    "a continuation line's indentation is a token inside the item's paragraph" in {
      blocks("- a\n  b\n").head match
        case MdCstNode.BulletList(_, _, children, _) =>
          val para = children.head.childNodes.collect { case p: MdCstNode.Paragraph => p }
          assert(para.size == 1)
          assert(para.head.childNodes.exists {
            case MdCstNode.Token("  ", _) => true
            case _                        => false
    ...
    "a list inside a quote nests, marker bytes shared but owned once" in {
      blocks("> - a\n").head match
        case MdCstNode.BlockQuote(children, _) =>
          assert(children.exists {
            case _: MdCstNode.BulletList => true
            case _                       => false
          })
        case other => assert(false, s"expected BlockQuote, got $other")
    }
  ...
  "html and definitions" - {
    ...
    "an HTML block is typed with a text interior" in {
      blocks("<div>\nhi\n</div>\n").head match
        case MdCstNode.HtmlBlock(children, _) =>
          assert(children.forall {
            case _: MdCstNode.Text => true
            case _                 => false
          })
    ...
    "a link reference definition is its own node" in {
      blocks("[a]: /url\n").head match
        case MdCstNode.LinkReferenceDefinition(_, _) => assert(true)
    ...
    "a definition before a paragraph leaves the paragraph typed" in {
      ...
      assert(nodes.headOption.exists {
        case _: MdCstNode.LinkReferenceDefinition => true
        case _                                    => false
      })
      assert(nodes.exists {
        case _: MdCstNode.Paragraph => true
    ...
    "a reference paragraph stays a paragraph, resolution being lowering's job" in {
      blocks("[a]\n\n[a]: /u\n").toSeq match
        case Seq(MdCstNode.Paragraph(_, _), _, MdCstNode.LinkReferenceDefinition(_, _), _*) => assert(true)
  ...
  "graduated inlines" - {
    ...
    "emphasis keeps its delimiter and its run tokens" in {
      paragraphChildren("a *b* c\n").collectFirst { case e: MdCstNode.Emphasis => e } match
        case Some(MdCstNode.Emphasis(delimiter, strong, children, _)) =>
          assert(delimiter == '*')
          assert(!strong)
          assert(children.collect { case MdCstNode.Token(t, _) => t }.toSeq == Seq("*", "*"))
    ...
    "strong emphasis spends two delimiters a side" in {
      paragraphChildren("__b__\n").collectFirst { case e: MdCstNode.Emphasis => e } match
        case Some(MdCstNode.Emphasis(delimiter, strong, children, _)) =>
          assert(delimiter == '_')
          assert(strong)
          assert(children.collect { case MdCstNode.Token(t, _) => t }.toSeq == Seq("__", "__"))
    ...
    "a partially consumed run leaves its extra delimiter verbatim" in {
      paragraphChildren("*foo**\n").collectFirst { case e: MdCstNode.Emphasis => e } match
        case Some(MdCstNode.Emphasis(_, strong, children, _)) =>
          assert(!strong)
          assert(children.collect { case MdCstNode.Token(t, _) => t }.toSeq == Seq("*", "*"))
        case _ => assert(false, "expected an Emphasis node")
    }
    ...
    "a hard break keeps its spelling" in
      assert(paragraphChildren("a\\\nb\n").exists {
        case MdCstNode.HardBreak(children, _) =>
          children.collect { case MdCstNode.Token(t, _) => t }.mkString.startsWith("\\")
        case _ => false
      })

    "an escape is a backslash token and the literal character" in {
      paragraphChildren("a \\* b\n").collectFirst { case e: MdCstNode.Escape => e } match
        case Some(escape) =>
          escape.childNodes.toSeq match
            case Seq(MdCstNode.Token("\\", _), MdCstNode.Text("*", _)) => assert(true)
    ...
    "an entity keeps its raw spelling" in {
      paragraphChildren("a &amp; b\n").collectFirst { case e: MdCstNode.Entity => e } match
        case Some(entity) =>
          assert(entity.childNodes.toSeq match
            case Seq(MdCstNode.Token("&amp;", _)) => true
            case _                                => false)
    ...
    "a heading's code span is typed inside the heading" in {
      blocks("# uses `f`\n").head match
        case MdCstNode.AtxHeading(_, children, _) =>
          assert(children.exists {
            case _: MdCstNode.CodeSpan => true
            case _                     => false
          })
    ...
    "a multi-line code span in a quote keeps the interior marker as a token" in {
      blocks("> a `b\n> c` d\n").head match
        case MdCstNode.BlockQuote(children, _) =>
          val spans = children.flatMap(_.childNodes).collect { case c: MdCstNode.CodeSpan => c }
          assert(spans.size == 1)
  ...
  "graduated links and images" - {
    ...
    def firstLink(source: String): MdCstNode.Link =
      blocks(source).head.childNodes.collectFirst { case l: MdCstNode.Link => l } match
        case Some(link) => link
    ...
    "an inline link keeps its tokens, destination and title spelling" in {
      ...
      val tokens = link.childNodes.collect { case MdCstNode.Token(t, _) => t }
      assert(tokens.toSeq == Seq("[", "](", "\"", "\"", ")"))
      val texts = link.childNodes.collect { case MdCstNode.Text(t, _) => t }
    ...
    "an angle-bracketed destination keeps its brackets as tokens" in {
      ...
      assert(link.childNodes.collect { case MdCstNode.Text(t, _) => t }.toSeq == Seq("/my url"))
      assert(link.childNodes.collect { case MdCstNode.Token(t, _) => t }.contains("<"))
    }

    "a full reference link keeps its second label" in {
      val link = firstLink("[text][ref]\n\n[ref]: /url\n")
      assert(link.form == LinkForm.ReferenceFull)
      assert(link.childNodes.collect { case MdCstNode.Text(t, _) => t }.toSeq == Seq("ref"))
    }

    "a collapsed reference link is bracketed as written" in {
      val link = firstLink("[ref][]\n\n[ref]: /url\n")
      assert(link.form == LinkForm.ReferenceCollapsed)
      assert(link.childNodes.collect { case MdCstNode.Token(t, _) => t }.toSeq == Seq("[", "][]"))
    }
    ...
    "an image's alt is inline content in the CST" in {
      blocks("![alt `x`](/img)\n").head.childNodes.collectFirst { case i: MdCstNode.Image => i } match
        case Some(image) =>
    ...
    "an unresolved shortcut stays verbatim, not a link" in
      assert(blocks("[nope]\n").head.childNodes.forall {
        case _: MdCstNode.Verbatim => true
        case _                     => false
      })
  }
