package morphir.langkit.markdown.compiler.kyoui
 ...
 *
 * The Scala package is `…compiler.kyoui` rather than `…compiler.kyo.ui`, even though the module directory is
 * `compiler/kyo/ui`: a package segment named `kyo` would shadow Kyo's root package and break the `import kyo.*` this
 * file needs. The nested directory is what gives the artifact its `kyo-ui` suffix.
 *
