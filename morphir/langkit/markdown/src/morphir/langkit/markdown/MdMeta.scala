 ...
 * makes [[MdMeta.get]]'s cast safe by construction.
 */
 ...
 * for. [[MdNode.unpositioned]] strips the span and keeps the data: position is derived provenance, data is content the
 * author attached.
 */
final case class MdMeta(
  ...
  def updated[A](key: MetaKey[A], value: A): MdMeta = copy(data = data.updated(key, value))

object MdMeta:
  val empty: MdMeta          = MdMeta()
  def at(span: Span): MdMeta = MdMeta(Present(span))
