...
private[markdown] object InlineParser:
  ...
  def parse(
    ...
    ExtendedAutolinks.extendAutolinks(parseNested(text, sourceOffsetAt, definitions, notes), text, sourceOffsetAt)

  /**
   * The same parse without the extended-autolink post-pass, for content that is already inside a link.
   *
   * A link may not contain a link, so recognising one in a label would not nest — it would disqualify the label
   * outright, and `[foo www.example.com](/uri)` would stop being a link at all. GFM's own postprocessor skips the
   * subtree of a link for the same reason, which is why every recursive call here — a link's text, an image's alt —
   * comes through this door and only the block-level entry points come through [[parse]].
   */
  private def parseNested(
      text: String,
      sourceOffsetAt: Int => Int,
      definitions: Map[String, LinkDefinition] = Map.empty,
      notes: Maybe[InlineNotes] = Absent
  ...
  private def scanItems(
    ...
    while index < text.length do
      ...
      if char == '\\' && index + 1 < text.length && isAsciiPunctuation(text.charAt(index + 1)) then
        ...
        // and [[ExtendedAutolinks.extendAutolinks]] needs exactly that of every text node it scans. Kept in one
        // node, a single `\*` anywhere in a paragraph would cost every bare destination in it; kept apart, only the
        // escape itself is unmappable, and the prose on either side of it still is. The writer's escaper has the
        // mirror of this rule: see `nodeStart` in [[MdWriter.escapeText]].
        flushPending()
      ...
      case Present(close) =>
        ...
        def record(
          ...
          notes.foreach { collected =>
            val alt =
              if image then Present(parseNested(label, index => sourceOffsetAt(open + index), definitions, notes))
              else Absent
        ...
        def build(
          ...
          if image then Present((end, MdNode.Image(uri, title, plainText(label, definitions), MdMeta.at(span))))
            ...
            val content = parseNested(label, index => sourceOffsetAt(open + index), definitions, notes)
   ...
   */
  private[internal] def normalizeUri(value: String): String =
    val safe = "-_.+!*'(),%#@?=;:/&$~"
    val out  = StringBuilder()
  ...
  private def plainText(label: String, definitions: Map[String, LinkDefinition]): String =
    parseNested(label, identity, definitions).map(plainOf).mkString
  ...
  private[internal] def spanOf(start: Int, end: Int, sourceOffsetAt: Int => Int): Span =
