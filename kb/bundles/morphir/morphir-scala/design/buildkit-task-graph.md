...
2. **Node outcome.** Five statuses:
   ...
   when the arm it picked then fails (the arm's own node carries the `Failed`). A fan-out node may close
...
6. **Progress.** The executor emits `Emit[PipelineEvent]` with a minimal vocabulary: run started and finished,
   node started and finished with status, optional node progress. Pairing runs one way: every node-started event
   closes with exactly one node-finished event, and every started node closes even on halt, so events alone
   reconstruct final state. A node-finished event can arrive with no matching start: a node that was skipped,
   cancelled, or blocked without ever running closes this way. Events carry `NodeId`
