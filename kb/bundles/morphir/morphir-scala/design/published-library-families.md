...
The shared sources hold the OKF model (bundle, concept, frontmatter) and depend on `langkit.markdown` for bodies, which [0033](../../../intent/0033-markdown-compilation.md) narrows to `langkit.markdown.core`; okf parses bodies and does not compile them, so it never pulls in `kyo-ui`. `Concept.parse` parses the whole file under an `MdProfile` that recognizes YAML frontmatter and reads the block off the parsed root; it split the leading fence itself at first, and that split now belongs to the parser. Frontmatter accessors are permissive (`Maybe`). `Bundle.parse` loads from in-memory files; the root `index.md` must carry `okf_version`. GitHub ingest depends on `connector.github` and also lives in shared sources, so it follows the connector onto JS and Native. JVM-only pieces, if any appear, go in `jvm/src`.
